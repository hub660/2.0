...
try:
    with open(f'{weizhi}{os.sep}data{os.sep}config.json', 'r',encoding="utf-8") as file:
        ...
        client = Deepseek(api_key=config['Ai_peport']['Deepseek_key'])
        model=config['Ai_peport']['Deepseek_model']
except:
...
class Xixunyun_report_Ai:
    ...
    def generate_report(self, standing, days_diff):
        messages = [
            {"role": "user", "content": f"实习岗位：{standing}\n{self.request1}"}
        ]
